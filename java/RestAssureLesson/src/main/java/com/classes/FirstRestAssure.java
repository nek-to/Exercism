...
//        getResponseContentType();
    ...
    public static void getResponseStatus() {
        ...
        given().when().get("http://demo.guru99.com/V4/sinkministatement.php").then().assertThat().statusCode(500);
    }
    ...
    public static void getSpecificFieldsOfResponseBody(){
        ArrayList<String> amount = when().get(url).then().extract().path("result.statements.AMOUNT");
        int sumOfAll = 0;
        for (String a : amount){
            System.out.println("The amount value fetched is " + a);
            sumOfAll +=Integer.parseInt(a);
        }
